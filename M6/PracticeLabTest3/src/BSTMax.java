...
public class BSTMax
{
    ...
    static class BinarySearchTree<T extends Comparable<T>>
    {
        ...
        public T max()
        {
            T max = this.root.element;

            ArrayList<Node> nodesSearch = new ArrayList<>();

            nodesSearch.add(this.root);

            while (!nodesSearch.isEmpty())
            {
                Node first = nodesSearch.remove(0);

                if (first.element.compareTo(max) > 0)
                {
                    max = first.element;
                }
                ...
                if (first.left != null)
                {
                    nodesSearch.add(first.left);
                }
                if (first.right != null)
                {
                    nodesSearch.add(first.right);
                }
            }
